...
object Production {
  ...
  def e2eFlowWithExpectations[A, B](sources: List[Source[A]],
                                  udfs: List[FlowUDF[B]] = List(),
                                  transformations: List[Transformation[A]],
                                  columnDSL: Map[String, Array[(ColumnExpectation, Boolean)]]): FreeFlowDSL[Map[String, (DataFrame, Map[String, ExpectationResult])]] = {

    for {
      _ <- loadSources[A](sources)
      _ <- loadUserDefinedFunctions[B](udfs)
      _ <- runTransformations[A](transformations)
      expectationResult <- runColumnExpectation[Map[String, (DataFrame, Map[String, ExpectationResult])]](columnDSL)
